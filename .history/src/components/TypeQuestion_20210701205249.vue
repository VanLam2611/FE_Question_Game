...
export default {
  ...
  methods: {
    ...
    getCorrect() {
      let key = this.questionId.correct;
      let t = "\n      " + key + "\n    ";
    ...
    getQuestion: function ($id) {
      axios
        .get("http://localhost/blog_question/blog/public/api/type/" + $id)
    ...
    getQuestionById: function ($id, index, type) {
      axios
        .get("http://localhost/blog_question/blog/public/api/home/" + $id)
        .then((response) => (this.questionId = response.data));
      axios
        .get("http://localhost/blog_question/blog/public/api/player")
        ...
        let times = setInterval(() => {
          ...
          if (second == 0) {
            ...
            if (this.check == true) {
              axios
                .post(
                  "http://localhost/blog_question/blog/public/api/point/" +
                    this.player[0].id,
                  {
  ...
  created() {
    axios
      .get("http://localhost/blog_question/blog/public/api/type")
