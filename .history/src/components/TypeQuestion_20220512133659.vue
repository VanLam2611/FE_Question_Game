    ...
    getCorrect () {
      ...
      temp.forEach((e) => {
        e.addEventListener('click', () => {
          let t = '\n     ' + this.questionId.correct + '\n     '
          for (let i = 0; i < temp.length; i++) {
            temp[i].style.background = 'darkcyan'
          }
          e.style.background = 'green'
          console.log('Chon: '+ t.length)
          console.log('Right: '+ typeof e.textContent)
          console.log('Right: '+  e.textContent)
          console.log('Right: '+  e.textContent.length)
          if (e.textContent === t) {
            this.check = true
            console.log('win')
            return
